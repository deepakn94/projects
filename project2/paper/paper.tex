...
Since $T_n$ is only defined for even $n$, henceforth we shall refer to $T_n$ as $T_{2m}$ where $m$ is any integer greater than or equal to $0$.\todo{0 or 1 here?}
...
Then it suffices to check that $y$ is to the right of $y-1$ and that $z$ is still to the right of $z-1$. This is easy: since $y-1$ and $z-1$ are odd numbers less than $x$ another odd number, they must be to the left of $x$, and hence to the left of $y$ and $z$.
...
Before we do so, however, we establish a process to transform an element $a \in A$ to an element $b \in B$.
...
Where the last permutation in the chain $\pi_k$ does not have a single three-element sub-sequence in the relative ordering $321$, that is $\pi' = \pi_k$ is a permutation in $T_n$ that avoids $321$.\todo{why does this terminate? also, why is this necessary?}
...
\todo{this is a really long sentence}The above two claims show that there is actually an easy way to transform an arbitrary permutation $a \in A$ to a $b \in B$, and similarly, to transform an arbitrary permutation $b \in B$ to a $a \in A$ -- to obtain a permutation in $B$ from a permutation $a \in A$, for every element $i$ in the permutation $a$, all even elements in $a$ to the right of $i$ less than $i$ should be reversed (that is reordered to in increasing order); similarly to obtain a permutation in $A$ from a permutation $b \in B$, for every element $i$ in the permutation $b$, all even elements in $b$ to the right of $i$, less than $i$ should be reversed (that is reordered to in decreasing order). \todo{should have some justification here that reversing reordering these elements doesn't 'step on each other'... that is, you can always do it for some $i_1$ and then some $i_2$ afterward}
...
Note that even though we proved that the sequences $t_n(231)$ and $t_n(213)$ are both equivalent, the methods used to prove that each sequence is equal to the sequence $1, 2, 4, 8, \ldots$ are vastly different -- this is an interesting topic of research that we would like to pursue as future work.\todo{is this necessary? it seems frantic. Maybe if we're going to do this, we should also go back to the point about the $s_n$ proofs being different and say a little more}
