...
In this paper we will investigate pattern avoidance in permutations of finite sets of positive integers.
...
\begin{definition}
For a permutation $\pi$, $s_n(\pi)$ represents the number of permutations in $S_n$ that avoid $\pi$. 
\end{definition}
...
\subsection{Flipping}
Another concept that we will use in this paper is the concept of flipping. We define the \emph{flip} of a sequence $a$ as the sequence $b$ with the same elements as $a$, but with elements in the opposite order. More concretely, if $a = a_1, a_2, \dots, a_n$ for some arbitrary $n$, then the flip of the sequence $a$ will be the sequence $b = a_n, a_{n-1}, \dots, a_2, a_1$.

We will talk about how the flip of a sequence is important to this avoidance problem later in the paper.
...
More interesting is the equivalences between $213$ and $231$, and $312$ and $321$. Before proving these equivalences, we state and prove the following lemmas which will be useful in our proof.
...
Now if $\pi$ does not avoid $321$, there must exist some sub-sequence $x, y, z$ in $\pi$ in the relative ordering $321$. From Lemma \ref{swapping_lemma}, we see that swapping $y$ and $z$ in the permutation $\pi$ yields another valid permutation in $T_n$ -- note that the permutation produced after this swap has $x, y, z$ in the relative ordering $312$ instead of $321$. If we do this for every sub-sequence in the relative ordering $321$, we see that we will produce a permutation in $T_n$ that avoids $321$. This process is completely invertible, since we can take a permutation $\pi'$ in $T_n$ that avoids $321$, and similarly swap all elements that violates $\pi'$'s avoidance of $321$ to obtain a permutation that avoids $312$. \textbf{Make this more rigorous}
