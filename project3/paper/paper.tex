...
\begin{lemma}
Let $A=(a_{i,j})$ be an $n\times n$ real symmetric matrix with $a_{i,i}=1$ for all $i$ and $|a_{i,j}|\leq \sqrt{\epsilon}$ for all $i\neq j$. Let $d$ be the rank of $A$. Then
...
\begin{theorem}
If $N=n+1$, then the minimum value of $\epsilon$ for $i\neq j$ is $\frac{1}{n^2}$. This $\epsilon$ is achieved when the vectors are arranged in an $n$ dimensional simplex centered at the origin.
\label{simplex}
\end{theorem}
...
Our results on this section concern the case when $N=n+1$. We shall see, in fact, that the optimal value of $\delta$ is given by $-\frac{1}{n}$, we introduce the parallelogram law.
