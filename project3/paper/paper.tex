...
\end{lstlisting}
\captionof{lstlisting}{The annealing algorithm used to generate optimum epsilon values}
...
We can use a well-known lemma to provide a useful lower bound on $\epsilon$. Our presentation of the result follows that of a paper by Noga Alon\footnote{Lemma 2.2, \emph{Perturbed identity matrices have high rank: proof and applications}, Noga Alon.}.
...
\end{lemma}
\begin{proof}
We define $\lambda_1,\ldots, \lambda_n$ to be the eigenvalues of $A$; write $k$ as the number of nonzero eigenvalues. Their sum is the trace of $A$, equal to $n$. Recall that the number of nonzero eigenvalues of a complex matrix is less than or equal to the rank of the matrix: so we have that $d\geq k$.
...
\[\epsilon\geq \frac{N-n}{n(N-1)}.\]
\end{proof}
For $N=n+1$, for instance, the theorem provides us a bound of 
\[\epsilon\geq \frac{(n+1)-n}{n(n+1-1)}=\frac 1{n^2}\]
implying that the $n$-simplex is indeed an optimal configuration. Similarly, for $n=3, N=6$, the theorem gives us
\[\epsilon\geq \frac{6-3}{3(6-1)}=\frac{1}{5}.\]
Again, the configuration we obtained was optimal. We present now a table of the bounds the theorem gives to us for varying $n$ and $N$:
\begin{table}[h]
   \centering
...
Note that the bounds for $n=2$ are not tight, as we proved in Section 2 that the minimum $\epsilon$ was given by $\cos^2(\pi/N)$. One direction for further research would be checking whether the bounds provided by Theorem \ref{lower_bound} are indeed tight for other combinations of $n$ and $N$.
