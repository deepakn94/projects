...
where $i, j \in \{1,2,\ldots,6\}$ -- the above result holds because $|v \cdot v'| = |(-v) \cdot v'|$ for some arbitrary vector $v'$.

Now, without loss of generality, let $x_1 = [1, 0]^T$; then since $x_4$ is $-x_1$, we see that $x_4 = [-1, 0]^T$. Also, without loss of generality let us assume that $x_2$ and $x_3$ are above the $x$-axis, and that $x_2$ is to the right of $x_3$.
...
$$
In our case we know three things. First, since the vectors are arranged in a simplex with centroid as the origin, $v_1+\cdots+v_{n+1}=0$. Second, $v_i\cdot v_j\le\delta$ for any $i<j$. Third, $||v_i||=1$. Therefore 
$$
