...
\subsection{n=3,N=6}
In this situation we have a lower bound of $\epsilon\ge\frac{1}{5}$. Consider the following 6 vectors on the unit sphere in $\R^3$ with variables $a$ and $b$. 
$$
(a,b,0),(a,0,b),(b,0,a)
$$
$$
(a,-b,0),(a,0,-b),(-b,0,a)
$$
There are four possible dot products we get from these six vectors: $1,ab,-ab,a^2-b^2$. This gives us three possible values of $\epsilon$: $1,(a^2-b^2)^2,(ab)^2$. Let us consider the solution we get when $a^2-b^2=ab$. If we solve the resulting system according to the constraint $a^2+b^2=1$, we get
$$
a=\sqrt{\frac{1}{10}(5-\sqrt{5}}, b=-\frac{1+\sqrt{5}}{2}\sqrt{\frac{1}{10}(5-\sqrt{5}}. 
$$
If we compute $\epsilon=(a^2-b^2)^2=(ab)^2$, we have that $\epsilon=\frac{1}{5}$, implying that these six points give us an optimal configuration.
\subsection{N=n+1}
Let us consider the situation in which we try to minimize $\epsilon$ for $n+1$ vectors in an $n$ dimensional space. From Theorem ~\ref{lower_bound} we know that $\epsilon\ge \frac{1}{n^2}$, so we will find a configuration of vectors that achieves this lower bound. First we introduce the definition of a simplex, and then we will proceed to characterize $\epsilon$. 
...
We now look to calculate the optimum value of $\epsilon$ for the specific case of $N = n+1$. Before proving an upper bound on what the optimum value of $\epsilon$ can be, we introduce the parallelogram law.
...
Let us consider the situation in which we try to minimize $\delta= \max_{i\neq j}\{v_i\cdot v_j\}$ for $n+1$ vectors in an $n$ dimensional space.
